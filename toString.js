class TOSTRING {
  CONFIG;
  THIS_METH_KEY_SPECIAL = ["isObject", "isArray", "isDate", "isFunction", "isRegExp"];
  MKSTRING_KEY=['Object', 'Array']
  constructor(conf) {
    this.CONFIG = conf;
  }
  // conf type
  ...
  recursionData(data) {
    if (
      this.MKSTRING_KEY.some(it=>`[object ${it}]`=== Object.prototype.toString.call(data))
    ) {
      // special datas
      return this.specialDataString(data, this.THIS_METH_KEY_SPECIAL.slice(0,2));
    }
    // keep string data 
  ...
  specialDataString(data, keys) {
    ...
    for (const meth of thisMeth) {
      if (this[meth] && this[meth](oo)) {
        ...
        return this[thisMethToStr] ? this[thisMethToStr](oo) : oo;
      }
    }
  }
